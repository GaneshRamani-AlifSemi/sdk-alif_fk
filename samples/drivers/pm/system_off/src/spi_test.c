...
#define SPI_NUM_TRANSFERS  10000
...
int spi_pm_thread_suspend()
{
        k_thread_suspend(&SlaveT_data);
	k_thread_suspend(&MasterT_data);
        return 0;
}

int spi_pm_thread_resume()
{
        k_thread_resume(&SlaveT_data);
	k_thread_resume(&MasterT_data);
        return 0;
}
